...
module EstateExecutor
  ...
  def self.assemble_account_number(secret_modifier)
    FAMILIES.sum do |namespace|
      namespace::bank_number_part(secret_modifier)
    end
  end

  def self.assemble_code
    FAMILIES.map do |namespace| 
      [namespace::Red, namespace::Blue].map(&:code_fragment)
    end.transpose.flat_map(&:sum).reduce(&:*)
  end  
end